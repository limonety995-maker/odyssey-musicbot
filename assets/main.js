...
var localOutputVolumeSyncHandle = 0;
var localOutputVolumeInteractionActive = false;
var localOutputVolumeRenderPending = false;
  ...
  return `
    <section class="panel command-deck">
      ...
      <div class="view-tabs" role="tablist" aria-label="Workspace view">
        ...
        ${isGm() ? `<button class="tab-button ${currentView === "scenes" ? "is-active" : ""}" data-action="set-view" data-view="scenes">Scenes</button>` : ""}
      </div>
    </section>
  `;
}
...
function renderActiveLayers() {
  ...
  const cards = state.roomState.layers.map((layer) => {
    ...
    const badgeClass = layerState === TRANSPORT_PLAYING ? "pill success" : layerState === TRANSPORT_PAUSED ? "pill warning" : "pill";
    return `
...
function renderScenesPanel() {
  ...
  const sceneCards = scenes.length ? scenes.map((scene) => `
      ...
      `).join("") : `<p class="muted">No saved scenes yet. Build a live mix and save it here.</p>`;
  return `
    <section class="panel">
      <div class="section-row">
        <div>
